...
router.get(
  "/join-meet-conversations/:conversationId",
  isLoggedIn,
  async (req, res) => {
  ...
  async (req, res, done) => {
    try {
      ...
      if (!conversation.length) {
        req.flash(
          "error",
          "No such meet conversation exist! Try creating one!"
        );
        res.redirect("/meet-conversations");
      } else {
        ...
        if (foundConvo.length) {
          ...
          req.flash(
            "success",
            "You're successfully added to the meet-conversations!"
          );
          res.redirect("/meet-conversations");
        }
      }
...
router.get(
  "/join-personal-conversations/:friendId/:userId",
  isLoggedIn,
  async (req, res, done) => {
    try {
