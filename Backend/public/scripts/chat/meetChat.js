  ...
  if (data.activeConversationId == activeConversationId) {
    ...
    if (data.userId != userId) {
      ...
      incomingMessageImage.innerHTML =
        '<img src="https://pixinvent.com/modern-admin-clean-bootstrap-4-dashboard-html-template/app-assets/images/portrait/small/avatar-s-11.png" alt="User">';
    ...
    data.forEach((item) => {
      if (item.sender === userId) {
        ...
        receivedWithdateMessage.innerHTML = `<b>${
          names[item.sender]
        }</b><br><p>${item.text}</p> <span class="time_date_in"> ${dayjs(
          item.sentAt
        ).format("hh:mm A | MMM D")} </span> </div>`;
