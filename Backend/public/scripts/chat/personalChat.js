...
socket.on("newMessage", async (data) => {
  ...
  if (data.activeConversationId == activeConversationId) {
    ...
    if (data.userId != userId) {
      ...
      incomingMessageImage.innerHTML =
        '<img class="rounded-circle" src="https://pixinvent.com/modern-admin-clean-bootstrap-4-dashboard-html-template/app-assets/images/portrait/small/avatar-s-11.png" alt="User">';
      ...
      receivedWithdateMessage.innerHTML = `<b>${user.data.name}</b><br><p>${
        msg.text
      }</p> <span class="time_date_in">${dayjs(msg.sentAt).format(
        "hh:mm A | MMM D"
      )}</span> </div>`;
      ...
      sentMessage.innerHTML = `<b>You</b><br><p>${
        msg.text
      }</p> <span class="time_date_in">${dayjs(Date.now()).format(
        "hh:mm A | MMM D"
      )}</span>`;
...
axios.get("/personal-conversations/" + userId).then((res, err) => {
  ...
  data.forEach(async (item) => {
    ...
    chatImage.innerHTML =
      '<img class="rounded-circle" src="https://pixinvent.com/modern-admin-clean-bootstrap-4-dashboard-html-template/app-assets/images/portrait/small/avatar-s-8.png" alt="User">';
...
$("#people").on("click", ".chat_ib", function (e) {
  ...
  if (activeConversationId !== "")
    document
      .getElementById(activeConversationId)
      .parentNode.parentNode.classList.remove("active_chat");
  ...
  axios.get(`/messages/${convoId}`).then((res, err) => {
    ...
    data.forEach((item) => {
      if (item.sender === userId) {
        ...
        sentMessage.innerHTML = `<b>You</b><br><p>${
          item.text
        }</p> <span class="time_date_out">${dayjs(item.sentAt).format(
          "hh:mm A | MMM D"
        )}</span> </div>`;
        ...
        incomingMessageImage.className = "incoming_msg_img";
        incomingMessageImage.innerHTML =
          '<img class="rounded-circle" src="https://pixinvent.com/modern-admin-clean-bootstrap-4-dashboard-html-template/app-assets/images/portrait/small/avatar-s-26.png" alt="User">';
        let receivedMessage = document.createElement("div");
        receivedMessage.className = "received_msg";
        let receivedWithdateMessage = document.createElement("div");
        receivedWithdateMessage.className = "received_withd_msg";
        receivedWithdateMessage.innerHTML = `<b>${
          names[item.sender]
        }</b><br><p>${
          item.text
        }</p> <span class="time_date_in"> 11:01 AM    |    June 10</span> </div>`;
...
$(".write_msg").keydown((e) => {
  if (e.which == 13 && text.val().length !== 0) {
    // when pressed enter , send message
    sendMessage();
  }
});
...
function sendMessage() {
  ...
  socket.emit(
    ...
    { activeConversationId, userId, message: text.val(), fromMeet: false },
    () => {
      let msg = {
        text: text.val(),
      };
      ...
      sentMessage.innerHTML = `<b>You</b><br><p>${
        msg.text
      }</p> <span class="time_date_out">${dayjs().format(
        "hh:mm A | MMM D"
      )}</span> </div>`;
      outgoingMessage.appendChild(sentMessage);
...
async function joinNewPersonalChatModel() {
  ...
  axios
    .get(`/join-personal-conversations/${friendsId}/${userId}`)
    .then(async (res, err) => {
      if (res.data.status) {
        // ------------------ show all the conversations ------------------------- //
